...
# Set to "yes" to test without downloading/installing anything
DRY_RUN="${DRY_RUN:-no}"

# Wrapper function for commands that should be skipped in dry-run mode
dry_run_cmd() {
    if [[ "$DRY_RUN" == "yes" ]]; then
...
partition_disk() {
    ...
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} wipefs -af $disk"
        echo -e "${YELLOW}[DRY-RUN]${NC} sgdisk -Z $disk"
        if [[ "${CONFIG[uefi]}" == "yes" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} Create GPT partition table with ESP"
        else
            echo -e "${YELLOW}[DRY-RUN]${NC} Create MBR partition table"
        fi
        # Set fake partitions for dry run
        ...
        echo -e "${YELLOW}[DRY-RUN]${NC} boot_part=${CONFIG[boot_part]}, root_part=${CONFIG[root_part]}"
        return 0
    fi
...
setup_encryption() {
    ...
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} cryptsetup luksFormat ${CONFIG[root_part]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} cryptsetup open ${CONFIG[root_part]} cryptroot"
...
format_partitions() {
    ...
    if [[ "$DRY_RUN" == "yes" ]]; then
        if [[ "${CONFIG[uefi]}" == "yes" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} mkfs.fat -F32 ${CONFIG[boot_part]}"
        else
            echo -e "${YELLOW}[DRY-RUN]${NC} mkfs.ext4 -F ${CONFIG[boot_part]}"
        fi
        echo -e "${YELLOW}[DRY-RUN]${NC} mkfs.${CONFIG[filesystem]} $root_device"
        return 0
    fi
...
mount_filesystems() {
    ...
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} Mount $root_device to $MOUNTPOINT"
        if [[ "${CONFIG[filesystem]}" == "btrfs" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} Create btrfs subvolumes: @ @home @var @tmp @snapshots"
        fi
        if [[ "${CONFIG[uefi]}" == "yes" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} Mount ${CONFIG[boot_part]} to $MOUNTPOINT/boot/efi"
        else
            echo -e "${YELLOW}[DRY-RUN]${NC} Mount ${CONFIG[boot_part]} to $MOUNTPOINT/boot"
        fi
        return 0
    fi
...
install_base_system() {
    ...
    dry_run_cmd pacstrap -K "$MOUNTPOINT" $packages
    if [[ "$DRY_RUN" != "yes" ]]; then
        genfstab -U "$MOUNTPOINT" >> "$MOUNTPOINT/etc/fstab"
    else
        echo -e "${YELLOW}[DRY-RUN]${NC} genfstab -U $MOUNTPOINT >> $MOUNTPOINT/etc/fstab"
    fi
}
...
add_repos() {
    ...
    if [[ "$DRY_RUN" != "yes" ]]; then
        ...
        echo -e "${YELLOW}[DRY-RUN]${NC} Enable multilib in pacman.conf"
    fi
    ...
    if [[ "$DRY_RUN" != "yes" ]]; then
        ...
        echo -e "${YELLOW}[DRY-RUN]${NC} Add xerolinux repo to pacman.conf"
    fi
    ...
    if [[ "$DRY_RUN" != "yes" ]]; then
        ...
        echo -e "${YELLOW}[DRY-RUN]${NC} Add chaotic-aur repo (pacman-key + pacman -U)"
    fi

    dry_run_cmd arch-chroot "$MOUNTPOINT" pacman -Sy
}

configure_system() {
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} Configure timezone: ${CONFIG[timezone]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} Configure locale: ${CONFIG[locale]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} Configure keyboard: ${CONFIG[keyboard]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} Configure hostname: ${CONFIG[hostname]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} Enable NetworkManager"
        if [[ "${CONFIG[encrypt]}" == "yes" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} Configure encryption hooks in mkinitcpio"
        fi
        return 0
    fi
...
install_bootloader() {
    if [[ "$DRY_RUN" == "yes" ]]; then
        if [[ "${CONFIG[uefi]}" == "yes" ]]; then
            echo -e "${YELLOW}[DRY-RUN]${NC} grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=GRUB --removable"
        else
            echo -e "${YELLOW}[DRY-RUN]${NC} grub-install --target=i386-pc ${CONFIG[disk]}"
        fi
        echo -e "${YELLOW}[DRY-RUN]${NC} grub-mkconfig -o /boot/grub/grub.cfg"
        return 0
    fi
    ...
    if [[ "${CONFIG[uefi]}" == "yes" ]]; then
        ...
        arch-chroot "$MOUNTPOINT" grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=GRUB --removable
    else
...
create_user() {
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} Set root password"
        echo -e "${YELLOW}[DRY-RUN]${NC} Create groups: sys network scanner power cups libvirt realtime sambashare autologin rfkill lp users video storage kvm optical audio wheel adm falcond"
        echo -e "${YELLOW}[DRY-RUN]${NC} Create user: ${CONFIG[username]}"
        echo -e "${YELLOW}[DRY-RUN]${NC} Configure sudoers for wheel group"
        return 0
    fi
    ...
    local groups_to_create="sys network scanner power cups libvirt realtime sambashare autologin rfkill lp users video storage kvm optical audio wheel adm falcond"
    for grp in $groups_to_create; do
        arch-chroot "$MOUNTPOINT" groupadd -f "$grp" 2>/dev/null || true
    done

    arch-chroot "$MOUNTPOINT" useradd -m -G sys,network,scanner,power,cups,libvirt,realtime,sambashare,autologin,rfkill,lp,users,video,storage,kvm,optical,audio,wheel,adm,falcond -s /bin/bash "${CONFIG[username]}"
...
install_graphics() {
    ...
    if [[ -n "$packages" ]]; then
        dry_run_cmd arch-chroot "$MOUNTPOINT" pacman -S --noconfirm --needed $packages
    fi
    ...
    if [[ "$needs_nvidia_config" == "yes" ]]; then
        if [[ "$DRY_RUN" != "yes" ]]; then
            ...
            echo -e "${YELLOW}[DRY-RUN]${NC} Configure NVIDIA modules in mkinitcpio.conf"
            echo -e "${YELLOW}[DRY-RUN]${NC} arch-chroot $MOUNTPOINT mkinitcpio -P"
            echo -e "${YELLOW}[DRY-RUN]${NC} Configure NVIDIA in GRUB"
        fi
    fi
}
...
setup_swap_system() {
    case "${CONFIG[swap]}" in
        "zram")
            dry_run_cmd arch-chroot "$MOUNTPOINT" pacman -S --noconfirm zram-generator
            if [[ "$DRY_RUN" != "yes" ]]; then
...
EOF
                ...
                echo -e "${YELLOW}[DRY-RUN]${NC} Create zram-generator.conf"
            fi
            ;;
        "file")
            if [[ "$DRY_RUN" != "yes" ]]; then
                ...
                echo -e "${YELLOW}[DRY-RUN]${NC} Create 4GB swapfile"
            fi
            ;;
        "none")
            ;;
...
prepare_kde_installer() {
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} Download/copy xero-kde.sh to $MOUNTPOINT/home/${CONFIG[username]}/"
        return 0
    fi
...
run_kde_installer() {
    ...
    if [[ "$DRY_RUN" == "yes" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} arch-chroot $MOUNTPOINT bash /home/${CONFIG[username]}/xero-kde.sh"
    else

...
show_dialog() {
    dialog --title ">> XeroLinux DE Installer <<" --colors --yesno "\nThis will install your choice of either \Zb\Z6KDE Plasma\Zn, \Zb\Z3Gnome\Zn or \Zb\Z5XFCE\Zn all Vanilla, no customizations. It will also add the \Zb\Z1XeroLinux\Zn and \Zb\Z4Chaotic-AUR\Zn repos, incl. some quality-of-life apps.\n\n\Zb\Z4Proceed at your OWN RISK!.\Zn" 11 70
    response=$?
...
main_menu() {
  CHOICE=$(dialog --stdout --title ">> Desktop Selection <<" --menu "\nChoose which DE to install :" 12 60 4 \
    1 "Plasma  : Install Plasma Desktop." \
    2 "GNOME   : Install Gnome Desktop." \
    3 "XFCE    : Install XFCE Desktop.")
  ...
  case "$CHOICE" in
    ...
    3)
      ...
      sleep 2
      systemctl enable lightdm.service && systemctl enable power-profiles-daemon.service
      ;;
    *)
      if [ "$CHOICE" == "" ]; then
        clear
...
systemctl enable bluetooth.service sshd

echo "Installing Printer & Scanner stuff..."
install_packages "hplip print-manager printer-support scanner-support"
systemctl enable cups.socket saned.socket

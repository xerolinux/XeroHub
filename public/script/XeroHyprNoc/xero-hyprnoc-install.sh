...
preflight() {
    ...
    if [[ "${os_id}" != "arch" && "${os_like}" != *arch* ]]; then
        print_error "Requires an Arch-based distro."
        exit 1
    fi
...
ensure_aur_helper() {
    ...
    if [[ -n "${AUR_HELPER}" ]]; then
        if command -v "${AUR_HELPER}" &>/dev/null; then
            AUR_CMD="${AUR_HELPER}"; print_success "Found: ${AUR_HELPER}"; echo ""; return
        fi
        print_step "Installing ${AUR_HELPER} (from repo or AUR)..."
        if $SUDO_CMD pacman -S --needed --noconfirm "${AUR_HELPER}" 2>/dev/null; then
            AUR_CMD="${AUR_HELPER}"; print_success "${AUR_HELPER} installed."; echo ""; return
        fi
    fi

    if command -v yay  &>/dev/null; then AUR_CMD="yay";  print_success "Found: yay";  echo ""; return; fi
    if command -v paru &>/dev/null; then AUR_CMD="paru"; print_success "Found: paru"; echo ""; return; fi
...
setup_chaotic_aur() {
    ...
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        print_warning "Skipping Chaotic-AUR."; echo ""; return
    fi
...
install_aur_group() {
    ...
    local failed=() installed=0
    for pkg in "${pkgs[@]}"; do
        if "${AUR_CMD}" -S --needed --noconfirm "${pkg}" 2>/dev/null; then (( installed++ )) || true
        else failed+=("${pkg}"); fi
    done
    [[ ${#failed[@]} -gt 0 ]] && print_warning "[${group}] Skipped (${#failed[@]}): ${failed[*]}"
    print_success "[${group}] Done — ${installed} installed, ${#failed[@]} skipped."; echo ""; return 0
...
install_packages() {
    ...
    print_success "System updated!"; echo ""
    ...
    print_success "Databases updated."; echo ""
}
...
customization_prompts() {
    ...
    echo -e "${PURPLE}═══════════════════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "${CYAN}── WEB BROWSERS ──────────────────────────────────────────────────────────${NC}"; echo ""
    ...
    echo -e "  ${CYAN} 7)${NC} Mullvad Browser    ${CYAN} 8)${NC} Ungoogled Chromium"; echo ""
    echo -e "${GREEN}── SOCIAL & COMMUNICATION ──────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${GREEN} 9)${NC} Discord           ${GREEN}10)${NC} Vesktop            ${GREEN}11)${NC} Telegram"
    echo -e "  ${GREEN}12)${NC} ZapZap            ${GREEN}13)${NC} Ferdium"; echo ""
    echo -e "${PURPLE}── DEVELOPMENT TOOLS ───────────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${PURPLE}14)${NC} VSCodium          ${PURPLE}15)${NC} GitHub Desktop     ${PURPLE}16)${NC} Meld"
    echo -e "  ${PURPLE}17)${NC} Hugo"; echo ""
    echo -e "${YELLOW}── PASSWORD MANAGERS ───────────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${YELLOW}18)${NC} KeePassXC         ${YELLOW}19)${NC} Bitwarden"; echo ""
    echo -e "${BLUE}── CREATIVE & IMAGING ──────────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${BLUE}20)${NC} GIMP              ${BLUE}21)${NC} Krita              ${BLUE}22)${NC} Inkscape"; echo ""
    echo -e "${RED}── MEDIA & AUDIO ───────────────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${RED}23)${NC} MPV               ${RED}24)${NC} Spotify            ${RED}25)${NC} EasyEffects"
    echo -e "  ${RED}26)${NC} Tenacity          ${RED}27)${NC} Kdenlive"; echo ""
    echo -e "${CYAN}── OFFICE ──────────────────────────────────────────────────────────────${NC}"; echo ""
    echo -e "  ${CYAN}28)${NC} LibreOffice"; echo ""

    read -p ">> Your choices: " user_input
    for choice in $user_input; do
        case $choice in
             1) BROWSER="$BROWSER firefox" ;;           2) BROWSER="$BROWSER floorp" ;;
             3) BROWSER="$BROWSER brave-bin" ;;         4) BROWSER="$BROWSER librewolf" ;;
             5) BROWSER="$BROWSER zen-browser-bin" ;;   6) BROWSER="$BROWSER vivaldi-meta" ;;
             7) BROWSER="$BROWSER mullvad-browser-bin" ;; 8) BROWSER="$BROWSER ungoogled-chromium-bin" ;;
             9) SOCIAL="$SOCIAL discord" ;;            10) SOCIAL="$SOCIAL vesktop" ;;
            11) SOCIAL="$SOCIAL telegram-desktop" ;;   12) SOCIAL="$SOCIAL zapzap" ;;
            13) SOCIAL="$SOCIAL ferdium-bin" ;;        14) DEV="$DEV vscodium" ;;
            15) DEV="$DEV github-desktop" ;;           16) DEV="$DEV meld" ;;
            17) DEV="$DEV hugo" ;;                     18) PASS="$PASS keepassxc" ;;
            19) PASS="$PASS bitwarden" ;;              20) IMAGING="$IMAGING gimp" ;;
            21) IMAGING="$IMAGING krita" ;;            22) IMAGING="$IMAGING inkscape" ;;
            23) MUSIC="$MUSIC mpv" ;;                  24) MUSIC="$MUSIC spotify" ;;
            25) MUSIC="$MUSIC easyeffects" ;;          26) MUSIC="$MUSIC tenacity" ;;
            27) MUSIC="$MUSIC kdenlive" ;;             28) OFFICE="$OFFICE libreoffice-fresh" ;;
        esac
    done
    ...
    [[ -z "$BROWSER$SOCIAL$DEV$PASS$IMAGING$MUSIC$OFFICE" ]] && echo -e "  ${YELLOW}(none selected)${NC}"
    echo -e "${PURPLE}═══════════════════════════════════════════════════════════════════════${NC}"; echo ""
    read -p "Press Enter to begin installation..."
}

install_user_packages() {
    print_header; print_step "Installing user-selected packages..."; echo ""
    ...
    print_success "User-selected packages installed!"; echo ""
}
...
ensure_hyprland_session() {
    ...
    if [[ -f "${session}" ]]; then print_success "hyprland.desktop present."; echo ""; return; fi
    print_warning "hyprland.desktop not found — writing manually..."
    $SUDO_CMD mkdir -p /usr/share/wayland-sessions
    $SUDO_CMD tee "${session}" > /dev/null << 'SESSEOF'
[Desktop Entry]
Name=Hyprland
...
SESSEOF
    print_success "Wrote ${session}"; echo ""
}
...
configure_hyprland() {
    ...
    if [[ ! -f "${cfg}" ]]; then
        ...
        if [[ -f "${example}" ]]; then cp "${example}" "${cfg}"; print_success "Copied default config."
        else touch "${cfg}"; print_warning "No example config — created empty hyprland.conf."; fi
    else
    ...
    if grep -q "# xerohyprnoc" "${cfg}"; then
        print_success "XeroHyprNoc block already present — skipping."; echo ""; return
    fi
...
HYPREOF
    ...
    print_success "Hyprland config written."; echo ""
}
...
configure_portals() {
    ...
    mkdir -p "${conf_dir}"
    if [[ -f "${conf}" ]]; then print_success "hyprland-portals.conf exists — skipping."; echo ""; return; fi
    cat > "${conf}" << 'PORTEOF'
[preferred]
...
PORTEOF
    print_success "Portal config written to ${conf}"; echo ""
}
...
install_noctalia_polkit() {
    ...
    if [[ -d "${dest}" ]]; then print_success "Noctalia polkit-agent already installed — skipping."; echo ""; return; fi
    mkdir -p "${plugins_dir}"
    local tmp; tmp=$(mktemp -d)
        ...
        cat > "${plugins_json}" << 'PLJSON'
{
    ...
    "states": {"polkit-agent": {"enabled": true,"sourceUrl": "https://github.com/noctalia-dev/noctalia-plugins"}},
    "version": 2
}
PLJSON
        print_success "plugins.json written."
    fi
    echo ""
}
...
setup_sddm() {
    ...
    $SUDO_CMD pacman -S --needed --noconfirm sddm || { print_error "SDDM install failed!"; exit 1; }
    print_success "SDDM installed."; echo ""
    ...
    $SUDO_CMD tee /etc/sddm.conf > /dev/null << 'SDDMEOF'
[General]
InputMethod=
SDDMEOF
    $SUDO_CMD tee /etc/sddm.conf.d/xero_settings.conf > /dev/null << 'SDDMCONF'
[Autologin]
Relogin=false
Session=hyprland
User=
...
MinimumUid=1000
SDDMCONF
    print_success "SDDM configured."; echo ""
...
show_completion() {
    ...
    echo -e "${PURPLE}║${NC}  1. Run: hyprctl monitors                        ${PURPLE}║${NC}"
    echo -e "${PURPLE}║${NC}  2. Edit: ~/.config/hypr/hyprland.conf           ${PURPLE}║${NC}"
    ...
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then $SUDO_CMD reboot
    else echo -e "  Reboot when ready: sudo reboot"; echo ""; fi
}
